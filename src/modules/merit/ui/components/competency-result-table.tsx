...
interface LevelSelectCellProps {
  fieldName: `competencies.${number}.${"achievementOwner" | "achievementChecker" | "achievementApprover"}`;
  form: UseFormReturn<MeritEvaluation>;
...
export const CompetencyResultTable = ({
  form,
  index,  
  hasChecker,
  period,
  ...
  const achievementOwner = useWatch({
    control: form.control,
    name: `competencies.${index}.achievementOwner`,
  });

  const achievementChecker = useWatch({
    control: form.control,
    name: `competencies.${index}.achievementChecker`,
  });

  const achievementApprover = useWatch({
    control: form.control,
  ...
  const ownerLevel1st = period === Period.EVALUATION_1ST ? achievementOwner : (data[0]?.owner ?? null);
  const checkerLevel1st = period === Period.EVALUATION_1ST ? achievementChecker : (data[0]?.checker ?? null);
  const approverLevel1st = period === Period.EVALUATION_1ST ? achievementApprover : (data[0]?.approver ?? null);
  ...
  const ownerLevel2nd = period === Period.EVALUATION_1ST ? (data[1]?.owner ?? null) : achievementOwner;
  const checkerLevel2nd = period === Period.EVALUATION_1ST ? (data[1]?.checker ?? null) : achievementChecker;
  const approverLevel2nd = period === Period.EVALUATION_1ST ? (data[1]?.approver ?? null) : achievementApprover;
      ...
      <tbody>
        <tr className="border-b border-border bg-background even:bg-sidebar">
              ...
              {period === Period.EVALUATION_1ST && permissions.canPerformOwner ? (
                ...
                <div className="text-sm text-muted-foreground">
                  {period === Period.EVALUATION_1ST 
                    ? form.getValues(`competencies.${index}.result`) 
                    : data[0]?.result}
                </div>
              )}
            </div>
          ...
          <td className="align-top px-3 py-2 dark:last:border-none last:shadow-none border-r border-border">
            {period === Period.EVALUATION_1ST && permissions.canPerformOwner ? (
              <LevelSelectCell
                fieldName={`competencies.${index}.achievementOwner`}
                form={form}
          ...
          {hasChecker && (
            <td className="align-top px-3 py-2 dark:last:border-none last:shadow-none border-r border-border">
              {period === Period.EVALUATION_1ST && permissions.canPerformChecker ? (
                <LevelSelectCell
                  fieldName={`competencies.${index}.achievementChecker`}
                  form={form}
          ...
          <td className="align-top px-3 py-2 dark:last:border-none last:shadow-none border-r border-border">
            {period === Period.EVALUATION_1ST && permissions.canPerformApprover ? (
              <LevelSelectCell
                fieldName={`competencies.${index}.achievementApprover`}
                form={form}
