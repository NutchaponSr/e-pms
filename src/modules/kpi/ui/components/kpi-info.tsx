              ...
              buttonCtx={{
                ...
                onClick: () => {
                  if (!isInRange(year, 1, 3, 2025)) {
                    toast.error("You can only define KPIs from January to March");
                    return;
                  }

                  if (!!form.task.draft) {
                    router.push(`/performance/kpi/${form.task.draft.id}/definition`);
                  } else {
                    createTask({
                      year,
                      type: FormType.KPI,
              ...
              buttonCtx={{
                ...
                onClick: () => {
                  if (!isInRange(year, 1, 12, 2025)) {
                    toast.error("You can only evaluate KPIs from January to December");
                    return;
                  }

                  if (!!form.task.evaluation) {
                    router.push(`/performance/kpi/${form.task.evaluation.id}/evaluation`);
                  } else {
                    createTask({
                      year,
                      type: FormType.KPI,
